...
void Camera::update(const float& deltaTime)
{
	...
	//lock camera pitch so it doesn't invert
	if (m_pitch < globals::MIN_CAM_PITCH) {
		m_pitch = globals::MIN_CAM_PITCH;
	}
	else if (m_pitch > globals::MAX_CAM_PITCH) {
		m_pitch = globals::MAX_CAM_PITCH;
	}
